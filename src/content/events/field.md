---
visibility: public_page
publishing_status: ready_for_review
program_line: theatre_performance
filters: [performance]
title:
  en: FIELD
  ro: Câmp
  ru: Поле
meta:
  category:
    en: Theatre Performance
    ro: Performance teatral
    ru: Театральный перформанс
summary:
  en: A contemporary dance performance by DUNE Dance Company about deep listening, shared attention and the invisible forces moving between bodies.
  ro: Un performance de dans contemporan de DUNE Dance Company despre ascultare profundă, atenție comună și forțele invizibile dintre corpuri.
  ru: Перформанс современного танца от DUNE Dance Company о глубоком слушании, общем внимании и невидимых силах между телами.
focus_label:
  en: Performance focus
  ro: Focus performativ
  ru: Фокус перформанса
focus:
  en: Deep listening · shared attention · resonance · collective body · transformation
  ro: Ascultare profundă · atenție comună · rezonanță · corp colectiv · transformare
  ru: Глубокое слушание · общее внимание · резонанс · коллективное тело · трансформация
body:
  en: >-
    FIELD creates a living stage space where movement, energy and consciousness meet and keep transforming one another. The dancers enter a magnetic field of tension, silence and impulse, allowing individual presence to shift toward collective resonance.
  ro: >-
    Câmp creează un spațiu scenic viu în care mișcarea, energia și conștiința se întâlnesc și se transformă continuu una pe alta. Dansatorii intră într-un câmp magnetic de tensiune, tăcere și impuls, lăsând prezența individuală să se deplaseze spre rezonanță colectivă.
  ru: >-
    Поле создаёт живое сценическое пространство, где движение, энергия и сознание встречаются и постоянно трансформируют друг друга. Танцовщики входят в магнитное поле напряжения, тишины и импульса, позволяя индивидуальному присутствию перейти в коллективный резонанс.
source_confidence: confirmed
localization_status:
  en: ready_for_review
  ro: native_edit_needed
  ru: ready_for_review
schedule:
  date_display: "Oct 1"
  time_display: "19:30 — 21:30"
occurrences:
  - date: "2026-10-01"
    date_label: "Oct 1"
    time_display: "19:30 — 21:30"
    sort_start: "2026-10-01T19:30:00"
location_ref: ginta-latina
people_label: By
people:
  - person_id: dune-dance-company
    display_role: company
    public_visibility: visible
credits:
  items:
    - label:
        en: Idea and direction
        ro: Idee și direcție
        ru: Идея и режиссура
      value:
        en: Petya Stoykova
        ro: Petya Stoykova
        ru: Petya Stoykova
    - label:
        en: Choreography and performance
        ro: Coregrafie și performance
        ru: Хореография и исполнение
      value:
        en: Radoslav Yordanov, Velizar Gruev, Ivan Nanyov, Daniel Kearns, Svilena Nikolova, Katina Dishkov, Silvia Cherneva
        ro: Radoslav Yordanov, Velizar Gruev, Ivan Nanyov, Daniel Kearns, Svilena Nikolova, Katina Dishkov, Silvia Cherneva
        ru: Radoslav Yordanov, Velizar Gruev, Ivan Nanyov, Daniel Kearns, Svilena Nikolova, Katina Dishkov, Silvia Cherneva
    - label:
        en: Music
        ro: Muzică
        ru: Музыка
      value:
        en: Tsvetan Momchilov
        ro: Tsvetan Momchilov
        ru: Tsvetan Momchilov
    - label:
        en: Costumes
        ro: Costume
        ru: Костюмы
      value:
        en: Petya Petrova
        ro: Petya Petrova
        ru: Petya Petrova
  full_source:
    en: Created with the financial support of the National Culture Fund.
    ro: Creat cu sprijinul financiar al National Culture Fund.
    ru: Создано при финансовой поддержке National Culture Fund.
media:
  hero: field-hero-placeholder
  gallery: []
related:
  events: []
  people: [dune-dance-company, daniel-kearns]
---

# FIELD
