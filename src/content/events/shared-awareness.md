---
visibility: public_page
publishing_status: ready_for_review
program_line: ci_performance
filters: [ci, performance]
title:
  en: Sharing Presence
  ro: Prezență împărtășită
  ru: Совместное присутствие
meta:
  category:
    en: CI & Performance class
    ro: Clasă de Improvizație de contact & performance
    ru: КИ и Перформанс класс
  format:
    en: 2 days
    ro: 2 zile
    ru: 2 дня
summary:
  en: A two-day Contact Improvisation & Performance class on presence as a meeting point for communication, perception and collective creation.
  ro: O clasă de două zile de Improvizație de contact & performance despre prezență ca punct de întâlnire pentru comunicare, percepție și creație colectivă.
  ru: Двухдневный класс по контактной импровизации и перформансу о присутствии как точке встречи для коммуникации, восприятия и коллективного творчества.
focus_label:
  en: Practice / performance focus
  ro: Focus practic / performativ
  ru: Фокус практики / перформанса
focus:
  en: Presence · somatic language · perception · collective creation · witnessing
  ro: Prezență · limbaj somatic · percepție · creație colectivă · martor activ
  ru: Присутствие · соматический язык · восприятие · коллективное творчество · свидетельствование
body:
  en: >-
    Sharing Presence studies Contact Improvisation as a form of communication applied to the performing arts. Through attention, perception, stillness and dynamic movement, the class approaches presence as a meeting point for contact, relation, collective creation and witnessing.
  ro: >-
    Prezență împărtășită studiază Improvizația de contact ca formă de comunicare aplicată artelor performative. Prin atenție, percepție, liniște și mișcare dinamică, clasa abordează prezența ca punct de întâlnire pentru contact, relație, creație colectivă și martor activ.
  ru: >-
    «Совместное присутствие» рассматривает контактную импровизацию как форму коммуникации, применённую к исполнительским искусствам. Через внимание, восприятие, тишину и динамичное движение класс подходит к присутствию как точке встречи для контакта, отношений, коллективного творчества и свидетельствования.
source_confidence: confirmed
localization_status:
  en: ready_for_review
  ro: native_edit_needed
  ru: ready_for_review
schedule:
  date_display: "Sep 26 — Sep 27"
  time_display: "12:00 — 14:30"
occurrences:
  - date: "2026-09-26"
    date_label: "Sep 26"
    time_display: "12:00 — 14:30"
    sort_start: "2026-09-26T12:00:00"
    location_ref: nod-space
  - date: "2026-09-27"
    date_label: "Sep 27"
    time_display: "12:00 — 14:30"
    sort_start: "2026-09-27T12:00:00"
    location_ref: nod-space
location_ref: nod-space
people_label: With
people:
  - person_id: daniel-rojasanta
    display_role: teacher
    public_visibility: visible
media:
  hero: shared-awareness-hero-placeholder
  gallery: []
related:
  events: [performative-space-enso]
  people: [daniel-rojasanta]
---

# Sharing Presence
